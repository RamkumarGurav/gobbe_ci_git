...
class Products extends Main
{
	...
	function all_products($main_cat = "" , $sub_cat = "" , $super_sub_cat = "" ,  $main_cat_slug='' , $sub_cat_slug='' , $super_sub_cat_slug='' , $manufacturer_id='' , $manufacturer_slug='')
	{
		...
		$this->data['breadcrumbs']='<li><i class="fa fa-home mr-5"></i><a href="'.base_url().'">Home&nbsp;</a></li><li><span>&nbsp;</span></li> ';
		$cat_search = "";
...
//		$this->data['refine_by'] = $this->data['refine_by'][0];
	...
	public function products_details($product_id , $combination_id='' , $main_cat = "" , $sub_cat = "" , $super_sub_cat = "" ,  $main_cat_slug='' , $sub_cat_slug='' , $super_sub_cat_slug='')
	{
		...
		else
		{
			...
			if(!empty($query_data))
			{
				...
				if($query_data1)
				{
					...
					if($query_data2)
					{
						...
						$this->data['breadcrumbs'].='<li></i><a href="'.base_url().base_url().$qd->slug_url.'/'.$qd1->slug_url.'/'.$qd2->slug_url.'" >'.$qd2->name.'</a></li><li><span>&nbsp;</span></li> ';
					}
				}
			...
			{
				$this->data['breadcrumbs'].='<li></i><a href="'.base_url().'all-products">All Products</a></li><li><span>&nbsp;</span></li> ';
		...
		$this->data['index_category'] = $this->Products_model->getIndexCategory(array('select'=>'id as category_id , position , name , slug_url ' , 'from'=>'category' , 'where'=>"(id =1 and status=1)" ));
		 $this->data['js'] = array('product-detail.js','star-rating.js','product.js','jquery.elevatezoom.js','slick.js'  );
		 $this->data['css'] = array('product-detail.css','rating-star.css');
		 $this->data['product_reviews_list'] = $this->Products_model->getList('product_reviews_list', $product_id, "10", "");
		 // print_r($this->data['product_reviews_list']);
		 // die;
		parent::getHeader('header' , $this->data);
		$this->load->view('product-detail' , $this->data);
		parent::getFooter('footer' , $this->data);
	}
	function doPostReview()
	{
		$msg="";
		if(empty($this->data['temp_name']) || empty($this->data['temp_id']) ){
			...
			$response_text = '<div class=" alert alert-danger">Please login to give rating and review!</div>';
			$response_code = 2;
		}
		else
		{
			...
			$entereddata['product_id']  = $_REQUEST['product_id'];
			$entereddata['product_combination_id']  = $_REQUEST['product_combination_id'];
			$entereddata['rating']  = $_REQUEST['rating'];
			$entereddata['customers_id']  = $this->data['temp_id'];
			$entereddata['customer_name']  = '';
			$entereddata['review_title']  = $_REQUEST['review_title'];
			$entereddata['review']  = $_REQUEST['review'];


			$entereddata['status'] = 0;
			$entereddata['added_on'] = date('Y-m-d H:i:s');
			$insertStatus = $this->Products_model->add($entereddata, 'product_reviews');
			if($insertStatus > 0 )	{



				$adm_msg ="<p align='left'>New Review Posted By - ".$this->data['profile']->name." Please verify the details.</p>";
				$adm_msg .= "<p align='left'><b>Review Title : </b>&nbsp;".$_REQUEST['review_title']."</p>\r\n";
				$adm_msg .= "<p align='left'><b>Review : </b>&nbsp;".$_REQUEST['review']."</p>\r\n";
				$adm_msg .= "<p align='left'><b>Rating: </b>&nbsp;".$_REQUEST['rating']."</p>\r\n";

				$mailMessage = file_get_contents(APPPATH . 'mailer/admin_msg.html');
				$mailMessage = preg_replace('/\\\\/', '', $mailMessage); //Strip backslashes
				$mailMessage = str_replace("#adm_msg#",$adm_msg,$mailMessage);

						//echo "$mailMessage <br>";

				$subject = _brand_name_." : New Review Posted By - ".$this->data['profile']->name;
			//	echo $mailMessage;
						//$address="abhishek.khandelwal82@gmail.com";
					$to=_project_email_;
			//	$mailStatus = $this->Common_Model->send_mail(array("template"=>$mailMessage , "subject"=>$subject , "to"=>$to , "name"=>_brand_name_));
					$response_text = '<div class="alert alert-success" role="alert">Thank You! Your review has been submitted for moderation</div>';
					$response_code = 1;
			}else{
					$response_text = '<div class=" alert alert-danger">Sorry! Some technical issue in submitting this input. Please try again.</div>';
					$response_code = 2;
			}
		}
		echo json_encode(array('response_text'=>$response_text , 'response_code'=>$response_code ));
	}
...
//				$dataaddcart['quantity']+=1;
	...
	function getCartDetail()
	{
		...
		/*else
		{
			...
			$showFooter = "</ul><div class='slideproceedButton'><ul><li><i class="fa fa-home mr-5"></i><a href='".base_url(__payment__)."' ><span>Proceed To Checkout </span><span>".$this->data['currency']->symbol."  $sub_total</span></a></li></ul></div></div></div>";
			//echo $display_header;
			//echo $display_body;
			//echo $showFooter;
		}*/
