...
class City_Module extends Main
{
	...
	function listing()
	{
		...
		$data_count = $this->City_Model->getData($search);
		...
		$this->data['list_data'] = $this->City_Model->getData($search);
		// echo $this->db->last_query();
		// die();
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/City_Module/listing' , $this->data);
		parent::get_footer();
	}
	...
	function export()
	{
		...
		$this->load->view('admin/master/City_Module/export' , $this->data);
	}
	function pdf()
	{
		...
		$html = $this->load->view('admin/master/City_Module/pdf' , $this->data,true);
		// echo $html;
		// exit;
	...
	function view($id="")
	{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['list_data'] = $this->City_Model->getData(array("id"=>$id));
		if(empty($id))
		{
			$alert_message = ' Something Went Wrong. Please Try Again.';
			$this->session->set_flashdata('alert_message', $alert_message);
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/City_Module/view' , $this->data);
		parent::get_footer();
	}
	...
	function edit($id="")
	{
		...
		$this->data['country_data'] = $this->Common_Model->getDropdownValues(array('select'=>'*' , 'from'=>'country' , 'where'=>" id > 0  and  status = 1 and is_deleted = 0",'order_by'=> 'name ASC'));
		$this->data['state_data'] = $this->Common_Model->getDropdownValues(array('select'=>'*' , 'from'=>'state' , 'where'=>" id > 0  and  status = 1 and is_deleted = 0",'order_by'=> 'name ASC'));
		...
		if(!empty($id)){
		 ...
		 $module_html = $this->load->view('admin/master/City_Module/edit' , $this->data,true);
	...
	function doEdit()
	{
		...
		$this->form_validation->set_rules('code', 'Code', 'numeric|trim|required|min_length[1]|max_length[6]');
		$this->form_validation->set_rules('country_id', 'Country', 'numeric|trim|required|min_length[1]|max_length[3]');
		...
		$this->form_validation->set_rules('name', 'Name', 'callback_alpha_dash_space|required');
			...
			//	print_r($is_exist);
				...
				$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
				$html = $this->load->view("admin/master/City_Module/edit",$this->data, true);
