...
class Coupon_Module extends Main
{
	function __construct()
	{
		...
		$this->data['page_module_name']='Coupons';
		$this->data['table_name']= 'coupon';
		$this->data['page_module_id'] = 75;
	...
	function listing()
	{
		...
		$data_count = $this->Coupon_Model->getData($search);
		...
		$this->data['list_data'] = $this->Coupon_Model->getData($search);
	...
	function pdf()
	{
		...
		$html = $this->load->view('admin/master/Coupon_Module/pdf' , $this->data,true);
		// echo $html;
		// exit;
	...
	function view($id="")
	{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['list_data'] = $this->Coupon_Model->getData(array("id"=>$id));
		if(empty($id))
		{
			$alert_message = ' Something Went Wrong. Please Try Again.';
			$this->session->set_flashdata('alert_message', $alert_message);
	...
	function edit($id="")
	{
		...
		if(!empty($id)){
		 ...
		 $module_html = $this->load->view('admin/master/Coupon_Module/edit' , $this->data,true);
	...
	function doEdit()
	{
		...
		if(!empty($id))
		{
			...
			$this->form_validation->set_rules('discount_in', 'Discount Type', 'required');
			...
			//	print_r($is_exist);
				...
				$enter_data['name'] = $name;
					...
					$enter_data['discount_value'] = $_POST['status'];
				if(isset($_POST['status'])){
				...
				$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
				$html = $this->load->view("admin/master/Coupon_Module/edit",$this->data, true);
