...
class Role_Manager_Model extends CI_Model
{
	...
	function getData($params = array())
	{
		...
		{
			$this->db->select("urm.*, aua.name as added_by_name, auu.name as updated_by_name");
		}

		$this->db->from("user_role_master as urm");
		$this->db->join("user as aua" , "aua.id = urm.added_by", "left");
		$this->db->join("user as auu" , "auu.id = urm.updated_by", "left");
		...
		//$this->db->where("urm.is_display" , 1);
