...
class Product_Reviews extends Main
{
	function __construct()
	{
		...
		$this->load->model('administrator/catalog/Product_Review_Model');
		...
		$this->data['page_module_name']='Product Reviews';
		$this->data['table_name']= 'product_reviews';
		$this->data['page_module_id'] = 74;
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/catalog/Product_Reviews/list' , $this->data);
		parent::get_footer();
	}
	...
	function listing()
	{
		...
		$data_count = $this->Product_Review_Model->getData($search);
		...
		$this->data['list_data'] = $this->Product_Review_Model->getData($search);
		// echo $this->db->last_query();
		// die();
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/catalog/Product_Reviews/listing' , $this->data);
		parent::get_footer();
	}
	...
	function export()
	{
		...
		$this->load->view('admin/catalog/Product_Reviews/export' , $this->data);
	}
	function pdf()
	{
		...
		$html = $this->load->view('admin/catalog/Product_Reviews/pdf' , $this->data,true);
		// echo $html;
		// exit;
	...
	function view($id="")
	{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['list_data'] = $this->Product_Review_Model->getData(array("id"=>$id));
		if(empty($id))
		{
			$alert_message = ' Something Went Wrong. Please Try Again.';
			$this->session->set_flashdata('alert_message', $alert_message);
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/catalog/Product_Reviews/view' , $this->data);
		parent::get_footer();
	}
	...
	function edit($id="")
	{
		...
		$this->data['attributes_inputs'] = $this->Common_Model->getDropdownValues(array('select'=>'*' , 'from'=>'attributes_input' , 'where'=>" id > 0  "));
		...
		if(!empty($id)){
		 ...
		 $module_html = $this->load->view('admin/catalog/Product_Reviews/edit' , $this->data,true);
	...
	function doEdit()
	{
		...
		if(!empty($id))
		{
			...
			$this->form_validation->set_rules('condition_per_product', 'No. Of Attributes Per Product', 'required');
			$this->form_validation->set_rules('attributes_input_id', 'Attribute Display Type', 'required');
			...
			//	print_r($is_exist);
				...
				$enter_data['condition_per_product'] = $_POST['condition_per_product'];
				$enter_data['attributes_input_id'] = $_POST['attributes_input_id'];
				...
				if(isset($_POST['search'])){
					$enter_data['search'] = $_POST['search'];
				}else{
					$enter_data['search'] = 0;
				}
				if(isset($_POST['list_page'])){
					$enter_data['list_page'] = $_POST['list_page'];
				}else{
					$enter_data['list_page'] = 0;
				}
				if(isset($_POST['details_page'])){
					$enter_data['details_page'] = $_POST['details_page'];
				}else{
					$enter_data['details_page'] = 0;
				}
				...
				$html = $this->load->view("admin/catalog/Product_Reviews/edit",$this->data, true);
		...
		if($user_access->update_module==1)
		{
			...
			if(!empty($id_arr) )
			{
				...
				if($task=="pdf")
				{
					$this->load->library('pdf');
				//	$this->data['list_data'] = $this->Common_Model->getData(array('select'=>'*' , 'from'=>$this->data['table_name'] , 'where'=>"id in ($ids)"));
					$this->data['list_data'] = $this->Common_Model->export(array('review_id'=>$id_arr,'table'=>$this->data['table_name']));
					$this->data['Module_name'] = 'Country';
					$html = $this->load->view('admin/catalog/Product_Reviews/pdf' , $this->data,true);
				  $action_taken = "Exported";
						$date = date('Y-m-d H:i:s');
				...
				$response = $this->Common_Model->update_operation(array('table'=>$this->data['table_name'] , 'data'=>$update_data , 'condition'=>"review_id in ($ids)" ));
				if($response){
