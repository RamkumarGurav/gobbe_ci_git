...
class Brand_Module extends Main
{
	function __construct()
	{
		...
		$this->data['page_module_name']='Brand';
		$this->data['table_name']= 'brand_master';
		$this->data['page_module_id'] = 69;
	...
	function listing()
	{
		...
		$data_count = $this->Brand_Model->getData($search);
		...
		$this->data['list_data'] = $this->Brand_Model->getData($search);
		// echo $this->db->last_query();
		// die();
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Brand_Module/listing' , $this->data);
		parent::get_footer();
	}
	...
	function export()
	{
		...
		$this->load->view('admin/master/Brand_Module/export' , $this->data);
	}
	function pdf()
	{
		...
		$html = $this->load->view('admin/master/Brand_Module/pdf' , $this->data,true);
		// echo $html;
		// exit;
	...
	function view($id="")
	{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['list_data'] = $this->Brand_Model->getData(array("id"=>$id));
		if(empty($id))
		{
			$alert_message = ' Something Went Wrong. Please Try Again.';
			$this->session->set_flashdata('alert_message', $alert_message);
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Brand_Module/view' , $this->data);
		parent::get_footer();
	}
	...
	function edit($id="")
	{
		...
		if(!empty($id)){
		 ...
		 $module_html = $this->load->view('admin/master/Brand_Module/edit' , $this->data,true);
	...
	function doEdit()
	{
		...
		if(!empty($id))
		{
			...
			//	print_r($is_exist);
				...
				$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
				$html = $this->load->view("admin/master/Brand_Module/edit",$this->data, true);
