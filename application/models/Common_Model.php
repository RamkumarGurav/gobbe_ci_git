...
class Common_Model extends CI_Model
{
	...
	function getCartItemCount()
	{
		...
		if(!empty($temp_id))
		{
			...
			->join('product as p' , "p.id = tc.product_id")
		...
		if(empty($cart_count)){$cart_count=0;}
	...
	function getWishlistItemCount()
	{
		...
		if(empty($wishlist_count)){$wishlist_count=0;}
	...
	function getMenu($params = array())
	{
		...
		$application_sess_store_id = $this->session->userdata('application_sess_store_id');
			$sql_get_list="select c.id as category_id ,  c.name ,  c.slug_url , c.cover_image , c.super_category_id from category as c where c.status=1 and c.super_category_id =0 order by ";
				$this->db
				->DISTINCT()
				->select('c.id as category_id ,c.name , c.slug_url , c.cover_image , c.super_category_id')
				->from('category as c')
	...
	function getDropdownValues($params = array()){
...
//		->join("client_group_category as cgc", "cgc.group_id = cg.group_id")
	...
	{
		$this->db
		->select('c.id as country_id , c.name as country_name , c.country_short_name , c.code as country_code')
		->from('country as c')
		->where('status' , 1);
		$result = $this->db->get();
		if($result->num_rows() > 0 )
