...
class Stock_Model extends Database_Tables

{
	...
	function getData($params = array())
	{
		...
		{
			$this->db->select("count(product_combination.id) over() as counts");
		}
		else
		{
			...
			product.ref_code
			");
			$this->db->select("product_combination.id as product_combination_id,
			product_combination.product_display_name as product_display_name,
			product_combination.ref_code as combi_product_combination,
			product_combination.added_on as added_on,
			product_combination.status as status
			");
			$this->db->select("product_in_store.id as product_in_store_id,
			product_in_store.price,
			product_in_store.final_price,
			product_in_store.discount,
			product_in_store.discount_var,
			product_in_store.updated_on,
			product_in_store.quantity,
			product_in_store.quantity_per_order,
			product_in_store.stock_out_msg
			");
			$this->db->select("group_concat( product_attribute.name , ': ', concat (product_combination_attribute.combination_value, ' ', product_attribute_value.name) SEPARATOR '~;') as combi
			");

			$this->db->select("brand_master.name as brand_name
			");

			$this->db->select("product_image.product_image_name
			");

		}
		$this->db->from("product_combination")
		->join("product", "product.id = product_combination.product_id")
		->join("brand_master", "brand_master.id = product.brand_id")
		->join("product_in_store", "product_in_store.product_combination_id = product_combination.id")
		->join("product_combination_attribute", "product_combination_attribute.product_combination_id = product_combination.id")
		->join("product_attribute", "product_attribute.id = product_combination_attribute.product_attribute_id")
		->join("product_attribute_value", "product_attribute_value.id = product_combination_attribute.product_attribute_value_id")
		->join("product_image", "product_image.id = product_combination.product_image_id")
		;
		$this->db->group_by("product_combination.id");

		if(!empty($params['product_id']))
		{
			$this->db->where("product.id" , $params['product_id']);
		}

		if(!empty($params['product_combination_id']))
		{
			$this->db->where("product_combination.id" , $params['product_combination_id']);
		}
	...
	function get_product_old($params = array())
