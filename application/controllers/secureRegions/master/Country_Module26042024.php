...
class Country_Module extends Main
{
	...
	function listing()
	{
		...
		$this->data['status'] = $record_status;

		$search['end_date'] = $end_date;
		$search['start_date'] = $start_date;
		$search['field_value'] = $field_value;
		$search['field_name'] = $field_name;
		$search['status'] = $record_status;
		...
		$this->data['list_data'] = $this->Country_Model->get_country($search);
	...
	function view($id="")
	{
		...
		if(empty($id))
		{
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fa fa-ban"></i> Something Went Wrong. Please Try Again. </div>';
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['list_data'] = $this->Country_Model->get_country(array("id"=>$id));
		if(empty($id))
		{
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fa fa-ban"></i> Something Went Wrong. Please Try Again. </div>';
			$this->session->set_flashdata('alert_message', $alert_message);
	...
	function edit($id="")
	{
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Country_Module/edit' , $this->data);
		parent::get_footer();
	}
	...
	function doEdit()
	{
		...
		if(isset($_POST['save']) || isset($_POST['save-add-new'])  || isset($_POST['save-add-new']))
		{
			...
			if ($this->form_validation->run() == true)
			{
				...
				$status = $_POST['status'];
				$is_exist = $this->Common_Model->getData(array('select'=>'*' , 'from'=>'country' , 'where'=>"name = '$name' and id != $id and is_deleted != 1"));
			//	echo $this->db->last_query();
			//	print_r($is_exist);
				if(!empty($is_exist))
				{
					$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fa fa-ban"></i> Country already exist in database.</div>';
					$this->session->set_flashdata('alert_message', $alert_message);
				...
				if(!empty($id))
				{
					...
					{
						$alert_message = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fa fa-check"></i> Record Updated Successfully </div>';
					}
				...
				else
				{
					...
					if(!empty($insertStatus))
					{
						$alert_message = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fa fa-check"></i> New Record Added Successfully </div>';
					}
		...
		$this->session->set_flashdata('alert_message', $this->data['message']);
		...
		if($user_access->update_module==1)
		{
			...
			if(!empty($id_arr))
			{
				...
				if($task=="export")
				{
					$this->load->library('PHPExcel');
					$this->load->library('PHPExcel/IOFactory');
				//	$this->data['list_data'] = $this->Common_Model->getData(array('select'=>'*' , 'from'=>'country' , 'where'=>"id in ($ids)"));
					$this->data['list_data'] = $this->Common_Model->export(array('id'=>$id_arr,'table'=>'country'));
					$this->data['Module_name'] = 'Country';
					$this->load->view('admin/master/Country_Module/export' , $this->data);
