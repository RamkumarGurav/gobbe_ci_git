...
class Login extends Main {
		...
		if($is_contact == 1)
		{
			...
			$name = "User";
			 ...
			 // $is_exist_otp = $this->_audl->check_exist_otp(array('contact'=>$contact , 'otp_for'=>'login_contact'));
				...
				// 	$otp_id = 	$is_exist_otp[0]->id;
				// 	$params['condition']="id = $otp_id";
			 // 	}
				$this->data['otp'] = $otp = $this->Common_model->random_password(6 , 'number');
				$params['otp']=$otp;
				$params['contact']=$contact;
				$params['otp_for']='login_contact';
				$this->_audl->add_new_otp_log($params);
			...
			$template = "Your "._brand_name_." verification code is {$otp}. "._project_web_;
	...
	public function check_contact_Otp($otp)
	{
		...
		if(!empty($is_exist_otp))
		{
			$contact = $is_exist_otp[0]->contact;
			$delete_operationArr = array(
				"table" => "otp_log",
				"where" => "(contact=$contact and otp_for = 'login_contact')"
			);
	...
	public function loginAuth()
	{
		if(isset($_POST['doLoginBTN']))
		{
			...
			$is_contact = 0;
				...
				if($login_type == 'otp'){
					$username = (!empty($_POST['contact_no']))?trim($_POST['contact_no']):'';
				}else{
					$username = (!empty($_POST['username']))?trim($_POST['username']):'';
				}
					if (filter_var($username, FILTER_VALIDATE_EMAIL)) {
