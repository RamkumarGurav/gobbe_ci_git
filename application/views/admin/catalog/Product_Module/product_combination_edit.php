...
<div class="tpa-conp-inner">
    ...
    <div class="tpci-row-one">
      <div class="tpci-ro-inner">
        ...
        <div class="tpci-roi-r">
          <span class="apc-conp-close"><i class="fa-solid fa-xmark"></i></span>
        </div>
      </div>
    </div>
  ...
  <div class="tpci-tab-con">
    ...
    <?php echo form_open(MAINSITE_Admin . "$user_access->class_name/doAddProductCombination", array('method' => 'post', 'id' => 'combination_form','autofocus'=>'autofocus', 'onsubmit' => 'return  savedoAddProductCombination(this)', "name" => "form", 'style' => '', 'class' => 'form-horizontal', 'role' => 'form', 'enctype' => 'multipart/form-data')); ?>
    ...
    <div class="row">
      ...
      <div class="tpc-cmb-box col-4">
        <div class="form-group">
          ...
          if (!empty($value)) {
            $select_product_attribute_id = $value;
          }
          if (empty($value)) {
            $value = $select_product_attribute_id;
          }
          $attributes = array(
            'name' => 'select_product_attribute_id',
            'id' => 'select_product_attribute_id',
            'title' => "Attribute Name",
            'onChange' => 'getAttValues(this.value,0)',
            'class' => 'form-control form-control-sm',


          );
          echo form_dropdown($attributes, $product_attributes);
          ?>
          <span>
      ...
      <div class="tpc-cmb-box col-4">
        <div class="form-group">
          ...
          if (!empty($value)) {
            $select_product_attribute_value_id = $value;
          }

          if (empty($value)) {
            $value = $select_product_attribute_value_id;
          }
          $attributes = array(
          ...
          echo form_dropdown($attributes, array(''=>'Select'));
          ?>
          <span>
      ...
      <div class="col-10">
        ...
        <label class="control-label" for="Name">
          Combinations
          <div data-title="Created Combinations." class="ico-help icon_title_box"><i class="fa fa-question-circle"></i>
          <div class="ques-popup">
                <span>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusamus, aspernatur!</span>
              </div>
          </div>
        </label>

        <ul id="list" style="display: inline-flex;">
          <?
          //print_r($product_combination_attribute_detail);
          if(!empty($product_combination_attribute_detail)){
            foreach ($product_combination_attribute_detail as $combination_attribute_detail) {
              ?>
              <li onclick="$(this).remove();" class="combination_list_items" style="color: rgb(0, 0, 255);"><?=$combination_attribute_detail->attribute_name?> :  <?=$combination_attribute_detail->attribute_value_name?>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <img width="22px" height="22px" src="http://mars-500/xampp/MARS1/gobbe_ci/assets/admin/img/cancel.png">
                <input type="hidden" name="product_attribute_id[]" value="<?=$combination_attribute_detail->product_attribute_id?>">
                <input type="hidden" name="combination_value[]" value="<?=$combination_attribute_detail->combination_value?>">
                <input type="hidden" name="product_attribute_value_id[]" value="<?=$combination_attribute_detail->product_attribute_value_id?>">
              </li>
              <?
            }
          }
      ...
      <div class="tpc-cmb-box col-4">
        <div class="form-group">
          ...
          echo form_label('Price Incl. GST', 'price');
      ...
      <div class="tpc-cmb-box col-12">
          <div class="form-group">
            <?
            echo form_label('Is Discount Available', 'Is Discount Available');

            $attributes = array(
              'name' => 'is_discount_available',
              'id' => 'is_discount_available',
              'value' => $value,

              'placeholder' => "Is Discount Available",
              'autofocus' => 'autofocus',
            ...
            echo form_input($attributes,''); ?>
            <span>
              <?php echo form_error('is_discount_available', '<div class="error" style="color: red">', '</div>'); ?>
            </span>

          </div>
      </div>
      <div class="row discount_fields" style="display:<?=$display?>">
        <div class="tpc-cmb-box col-4">
          <div class="form-group">
            <?php
        ...
        <div class="tpc-cmb-box col-4">
          ...
          <div class="form-group">
            ...
            echo form_label('Discounted Price Incl. GST', 'price');
            ...
            $attributes = array(
              ...
              'id' => 'discounted_price',
              'value' => $value,
              'placeholder' => "Discounted Price Incl. GST",
              'autofocus' => 'autofocus',
              'class' => 'form-control form-control-sm',
              'type' => 'text',
      ...
      <div class="tpc-cmb-box col-12">
        <label class="control-label" for="Name">
          ...
          <div data-title="Price Of This Combination." class="ico-help icon_title_box"><i
        ...
        <div class="tpc-cb-row">
          <div class="row">
            ...
            foreach ($product_image_detail as $col) {
              $pid_count++; ?>
              <div class="col-2 tpc-cbc">
                <div class="tpc-cbc-inner">
                  <div class="tpc-cbc-inimg">
                    <img src="<?= _uploaded_files_ ?>product/small/<? echo $col->product_image_name; ?>" >
                  </div>
                  <div class="tpti-radio">
                    <?
                    if($col->default_image==1){
                      $checked = 'checked';
                    }elseif ($pid_count==1) {
                      $checked = 'checked';
                    }else{
                    ...
                    <div class="checkbox-wrapper-19">
                      <input <?=$checked?> type="radio" name="product_image_id" id="product_image_id_<?=$pid_count?>"
                        value="<? echo $col->product_image_id; ?>"  required>
                      <label for="cbtest-<?=$pid_count?>" class="check-box">
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div class="auto_seo_combi_tags" style="display:<?= $displayseo ?>">
        ...
        <div class="tpc-cmb-box col-8">
          <input type="button" onclick="combinationValidation('generateMeta')" class="btn btn-primary"
            value="Generate SEO Tags" style="width: 100%;">
        </div>
        <div class="tpc-cmb-box col-4">
          <div class="form-group">
            <?php
...
<script>
  ...
  function replaceAll(str, find, replace) {
    ...
    function discountByPrice() {
      ...
      var discount_type = discount_var.value;

      if(discount_type == '%'){

        var discount_amount =   (price.value*discount.value)/100;
        var fds_price = price.value-discount_amount;
        discounted_price.value =  fds_price.toFixed(2);
        final_price.value = fds_price.toFixed(2);
      }else{
  ...
  //
  // }
  function getAttValues(select_product_attribute_id , id=0)
  {
  $('#loader1').show();
  $("#select_product_attribute_value_id").html( '' );
  if(select_product_attribute_id > 0)
  {

    $.ajax({
      url: "<?=MAINSITE_Admin.'Ajax/getAttValues'?>",
      type: 'post',
      dataType: "json",
      data: {	'select_product_attribute_id' : select_product_attribute_id , 'select_product_attribute_value_id' : id , "<?=$csrf['name']?>":"<?=$csrf['hash']?>" },
      success: function( response ) {
        $("#select_product_attribute_value_id").html( response.attribute_value_html );
      },
...
<script>
  ...
  $('#is_discount_available').click(function(){
