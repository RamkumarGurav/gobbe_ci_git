...
class Employee_Module extends Main
{
	...
	function listing()
	{
		...
		$data_count = $this->Employee_Model->getData($search);
		...
		$this->data['list_data'] = $this->Employee_Model->getData($search);
		// echo $this->db->last_query();
		// die();
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/human_resource/Employee_Module/listing' , $this->data);
		parent::get_footer();
	}
	...
	function export()
	{
		...
		$this->load->view('admin/human_resource/Employee_Module/export' , $this->data);
	}
	function pdf()
	{
		...
		$html = $this->load->view('admin/human_resource/Employee_Module/pdf' , $this->data,true);
		// echo $html;
		// exit;
	...
	function view($id="")
	{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['list_data'] = $this->Employee_Model->getData(array("id"=>$id));
		if(empty($id))
		{
			$alert_message = ' Something Went Wrong. Please Try Again.';
			$this->session->set_flashdata('alert_message', $alert_message);
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/human_resource/Employee_Module/view' , $this->data);
		parent::get_footer();
	}
	...
	function edit($id="")
	{
		...
		$this->data['staff_roles'] = $this->Common_Model->getDropdownValues(array('select'=>'*' , 'from'=>'user_role_master' , 'where'=>" id > 0  and  status = 1 and is_deleted = 0",'order_by'=> 'name ASC'));
		...
		if(!empty($id)){
		 ...
		 $module_html = $this->load->view('admin/human_resource/Employee_Module/edit' , $this->data,true);
	...
	function doEdit()
	{
		...
		$this->data['staff_roles'] = $this->Common_Model->getDropdownValues(array('select'=>'*' , 'from'=>'user_role_master' , 'where'=>" id > 0  and  status = 1 and is_deleted = 0",'order_by'=> 'name ASC'));
			...
			$this->form_validation->set_rules('mobile_no', 'Mobile Number', 'numeric|trim|required');
			$this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email');
			$this->form_validation->set_rules('password', 'Password', 'required');
			$this->form_validation->set_rules('joining_date', 'Joining Date', 'required');
			$this->form_validation->set_rules('role_id', 'Staff Role', 'required');
			...
			//	print_r($is_exist);
				...
				$enter_data['mobile_no'] = $_POST['mobile_no'];
				$enter_data['email'] = $_POST['email'];
				$enter_data['show_password'] = $_POST['password'];
				$enter_data['password'] = md5($_POST['password']);

				$enter_data['joining_date'] = $_POST['joining_date'];
				$enter_data['role_id'] = $_POST['role_id'];
				if(isset($_POST['status'])){
				...
				if(!empty($insertStatus)){
					//member image uploads
						...
						$timg_name = $_FILES['user_image']['name'];
						$temp_var = explode(".",strtolower($timg_name));
						//$temp_count = rand(10,10000);
						$file_path = 'assets/uploads/admin_user/user_image';
						if (!is_dir($file_path))
							mkdir("./$file_path", 0777, TRUE);
						$timage_ext = end($temp_var);
						$timage_name_new = $id.".".$timage_ext;
						$mp_data = array();
						$mp_data['user_image'] = $timage_name_new = $id.".".$timage_ext;
						//$timage_name_new = "file_".$quotation_enquiry_id.'_'.$count.".".$timage_ext;
						$imginsertStatus = $this->Common_Model->update_operation(array('table'=>$this->data['table_name'], 'data'=>$mp_data, 'condition'=>"id = $id"));

						if($imginsertStatus > 0){

							move_uploaded_file ($_FILES['user_image']['tmp_name'],"assets/uploads/admin_user/user_image/".$timage_name_new);
				...
				$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
				$html = $this->load->view("admin/human_resource/Employee_Module/edit",$this->data, true);
