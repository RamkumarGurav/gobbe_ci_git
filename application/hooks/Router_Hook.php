...
ga('send', 'pageview');location.href='$url';</script>";*/
	...
	//	print_r($segments);
		if(!empty($segments[0]))
		{
			...
			$sql = "SELECT id as category_id ,slug_url FROM category where status=1 and slug_url like '$segments[0]' and super_category_id = 0 "; // MAIN CATEGORY
	        $query = mysqli_query($con , $sql);
			while ($route = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			...
			{
				$isProduct = true;
				$sql = "SELECT id as category_id ,slug_url FROM category where status=1 and slug_url like '$segments[1]' and super_category_id = $category_id ";  // SUB MAIN CATEGORY
				$query = mysqli_query($con , $sql);
				while ($route = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
					$isProduct = false;
				...
				if($isProduct)
				{
					$sql = "SELECT product_seo.product_id , product_seo.product_combination_id , product_seo.store_id , product_seo.product_in_store_id	 ,product_seo.slug_url FROM product_seo , product , product_combination where product_seo.slug_url like '$segments[1]' and product.id = product_seo.product_id  and product.id = product_seo.product_id and product_combination.id = product_seo.product_combination_id and product.status=1 and product_combination.status=1 ";
					$query = mysqli_query($con , $sql);
					while ($route = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			...
			{
				$isProduct = true;
				$sql = "SELECT id as category_id ,slug_url FROM category where status=1 and slug_url like '$segments[2]' and super_category_id = $category_id1 ";  // SUPER SUB MAIN CATEGORY
				$query = mysqli_query($con , $sql);
				while ($route = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
					$isProduct = false;
				...
				if($isProduct)
				{
					...
					$sql = "SELECT product_seo.product_id , product_seo.product_combination_id , product_seo.store_id , product_seo.product_in_store_id	 ,product_seo.slug_url FROM product_seo , product , product_combination where product_seo.slug_url like '$segments[2]' and product.id = product_seo.product_id  and product.id = product_seo.product_id and product_combination.id = product_seo.product_combination_id and product.status=1 and product_combination.status=1 ";
					$query = mysqli_query($con , $sql);
					while ($route = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			...
			if(!empty($segments[3]))
			{
				...
				$sql = "SELECT product_seo.product_id , product_seo.product_combination_id , product_seo.store_id , product_seo.product_in_store_id	 ,product_seo.slug_url FROM product_seo , product , product_combination where product_seo.slug_url like '$segments[3]' and product.id = product_seo.product_id  and product.id = product_seo.product_id and product_combination.id = product_seo.product_combination_id and product.status=1 and product_combination.status=1 ";
				$query = mysqli_query($con , $sql);
				while ($route = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			...
			$sql = "SELECT product_seo.product_id , product_seo.product_combination_id , product_seo.store_id , product_seo.product_in_store_id	 ,product_seo.slug_url FROM product_seo , product , product_combination where product_seo.slug_url like '$segments[0]' and product.id = product_seo.product_id  and product.id = product_seo.product_id and product_combination.id = product_seo.product_combination_id and product.status=1 and product_combination.status=1 ";
			//echo $sql;
	        $query = mysqli_query($con , $sql);
			while ($route = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		/*$sql = "select sub_category_name , sub_category_id , category_id , (select c.category_name from category as c where c.id=sc.category_id) as category_name from sub_category as sc where sc.status=1";
        $query = mysqli_query($con , $sql);
