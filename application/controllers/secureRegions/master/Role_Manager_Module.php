...
class Role_Manager_Module extends Main
{
	function __construct()
	{
		...
		$this->data['page_module_name']='Role Manager';
		$this->data['table_name']= 'user_role_master';
	...
	function listing()
	{
		...
		$data_count = $this->Role_Manager_Model->getData($search);
		...
		$this->data['list_data'] = $this->Role_Manager_Model->getData($search);
		// echo $this->db->last_query();
		// die();
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Role_Manager_Module/listing' , $this->data);
		parent::get_footer();
	}
	...
	function export()
	{
		...
		$this->load->view('admin/master/Role_Manager_Module/export' , $this->data);
	}
	function pdf()
	{
		...
		$html = $this->load->view('admin/master/Role_Manager_Module/pdf' , $this->data,true);
		// echo $html;
		// exit;
	...
	function view($id="")
	{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['list_data'] = $this->Role_Manager_Model->getData(array("id"=>$id));
		if(empty($id))
		{
			$alert_message = ' Something Went Wrong. Please Try Again.';
			$this->session->set_flashdata('alert_message', $alert_message);
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Role_Manager_Module/view' , $this->data);
		parent::get_footer();
	}
	...
	function edit($id="")
	{
		...
		$this->data['module_data'] = $this->Common_Model->getData(array('select'=>'*' , 'from'=>'module_master' , 'where'=>"id >0"));
		$this->data['module_permission_data'] = $this->Common_Model->getData(array('select'=>'*' , 'from'=>'module_permissions' , 'where'=>"user_role_id = $id"));
		...
		if(!empty($id)){
		 ...
		 $module_html = $this->load->view('admin/master/Role_Manager_Module/edit' , $this->data,true);
	...
	function doEdit()
	{
		...
		if(!empty($id))
		{
			...
			//$this->form_validation->set_rules('module_ids', 'Modules', 'required');
			...
			//	print_r($is_exist);
				...
				if($insertStatus>0)
	{
		$this->Common_Model->delete_operation(array('table'=>'module_permissions' , 'where'=>"user_role_id = $id"));
		//delete_operation
		if(!empty($_POST['module_ids']))
		{
			...
			foreach($module_id_arr as $module_id)
			{
				...
				$module_permission_data['user_role_id'] = $id;
				...
				$html = $this->load->view("admin/master/Role_Manager_Module/edit",$this->data, true);
