...
class Customers_Module extends Main
{
	function __construct()
	{
		...
		$this->data['page_module_name']='Customers';
		$this->data['table_name']= 'customers';
		$this->data['page_module_id'] = 73;
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/customers/listing' , $this->data);
		parent::get_footer();
	}
	...
	function listing()
	{
		...
		$data_count = $this->Customers_Model->getData($search);
		...
		$this->data['list_data'] = $this->Customers_Model->getData($search);
	...
	function view($id="")
	{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['list_data'] = $this->Customers_Model->getData(array("id"=>$id));
		if(empty($id))
		{
			$alert_message = ' Something Went Wrong. Please Try Again.';
			$this->session->set_flashdata('alert_message', $alert_message);
	...
	function edit($id="")
	{
		...
		if(!empty($id)){
		 ...
		 $module_html = $this->load->view('admin/customers/edit' , $this->data,true);
	...
	function doEdit()
	{
		...
		if(!empty($id))
		{
			...
			//	print_r($is_exist);
				...
				$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
				$html = $this->load->view("admin/customers/edit",$this->data, true);
		...
		if($user_access->update_module==1)
		{
			...
			if(!empty($id_arr) )
			{
				...
				if($task=="pdf")
				{
					$this->load->library('pdf');
				//	$this->data['list_data'] = $this->Common_Model->getData(array('select'=>'*' , 'from'=>$this->data['table_name'] , 'where'=>"id in ($ids)"));
					$this->data['list_data'] = $this->Common_Model->export(array('customers_id'=>$id_arr,'table'=>$this->data['table_name']));
					$this->data['Module_name'] = 'Country';
					$html = $this->load->view('admin/customers/pdf' , $this->data,true);
				  $action_taken = "Exported";
						$date = date('Y-m-d H:i:s');
				...
				$response = $this->Common_Model->update_operation(array('table'=>$this->data['table_name'] , 'data'=>$update_data , 'condition'=>"customers_id in ($ids)" ));
				if($response){
