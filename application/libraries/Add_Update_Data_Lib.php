...
class Add_Update_Data_Lib
{
	...
	function add_new_otp_log($params = array())
	{
		...
		if(isset($params['condition'])){
			$data_to_add['updated_on'] = date('Y-m-d H:i:s');
			$inserted_id= $this->update_operation(array('table'=>'otp_log' ,'condition'=>$params['condition'], 'data'=>$data_to_add));
	...
	{
		if(empty($params)) return false;
		$this->CI->db->where($params['condition']);
		$status = $this->CI->db->update($params['table'], $params['data']);
		return $status;
	}
